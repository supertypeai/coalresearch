...
- News articles are scraped from multiple sources using a collection of scripts located in the `insider_news/` directory. The main orchestration is handled by `insider_news/scripts/pipeline.py`.
...
**Source:**
- Data is automatically scraped from the official ESDM Minerba auction portal (`https://minerba.esdm.go.id/lelang/`) by the `mining_license_auctions_scraper.py` script.
...
Stores annual financial data for companies, including assets, revenue, costs, and profit.

**Source:**
- Data for this table is sourced from company annual reports. It is manually entered into the `company_financials` tab of the [Insider Sheets](https://docs.google.com/spreadsheets/d/19wfJ2fc9qKeR22dMIO2rEQLkit8E4bGsHA1u0USqTQk/edit?gid=2112285298#gid=2112285298). The `sheet_api/company_financials.py` script then syncs this data into the `db.sqlite` database.
