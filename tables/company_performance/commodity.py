...
def get_json_columns(df: pd.DataFrame, commodity_type: str) -> list[str]:
    ...
    commodity_df = df.loc[
        df["commodity_type"].str.lower() == commodity_type.lower()
    ].copy()
...
def flatten_gold_data(data: dict) -> dict:
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            for sub_key, sub_value in value.items():
                renamed = (
                    sub_key.replace("reserves", "ore_reserves")
                    if "reserves" in sub_key
                    else sub_key
                )
...
def create_new_sheets(spreadsheet: gspread.Spreadsheet, df: pd.DataFrame) -> None:
    ...
    for new_sheet in COMMODITY_PERFORMANCE_SHEET:
        ...
        try:
            ...
            print(
                f"Sheet '{new_sheet_name}' already exists. Clearing and updating headers."
            )
            worksheet.clear()
        except gspread.WorksheetNotFound:
            print(f"Creating new sheet: '{new_sheet_name}'...")
            worksheet = spreadsheet.add_worksheet(
                title=new_sheet_name, rows="100", cols=len(final_headers)
            )
    ...
    for commodity_name in COMMODITY_PERFORMANCE_SHEET:
        ...
        for _, row in commodity_df.iterrows():
            try:
                ...
                if row.get("commodity_stats") and isinstance(
                    row.get("commodity_stats"), str
                ):
                ...
                print(
                    f"Warning: Could not process JSON for row id {row.get('id')}. Error: {e}. Skipping."
                )
                continue
        ...
        df_serializable["year_measured"] = df_serializable["year_measured"].str.replace(
            r"\.0$", "", regex=True
        )
...
def write_new_company_performance(
    spreadsheet: gspread.Spreadsheet, df: pd.DataFrame
) -> None:
    """
    ...
    try:
        ...
        if missing_cols:
            print(
                f"Error: The following required columns are missing from the source data: {missing_cols}"
            )
            return
        ...
        try:
            worksheet = spreadsheet.worksheet(new_sheet_name)
            print(
                f"Sheet '{new_sheet_name}' already exists. Clearing and writing new data."
            )
            worksheet.clear()
        except gspread.WorksheetNotFound:
            print(f"Creating new sheet: '{new_sheet_name}'...")
            worksheet = spreadsheet.add_worksheet(
                title=new_sheet_name, rows=len(df_new) + 1, cols=len(df_new.columns)
            )

        # Write the headers and the data to the new sheet
        worksheet.update(
            range_name="A1",
            values=[df_new.columns.values.tolist()] + df_new.values.tolist(),
        )
        print(
            f"Successfully wrote {len(df_new)} rows to '{new_sheet_name}' with performance_id column."
        )
