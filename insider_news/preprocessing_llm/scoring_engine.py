...
CRITERIA = """Revised Scoring System: Indonesian Coal, Metal & Mineral Market Intelligence
    News Article Scoring Criteria (0-100) but can have bonus point based bonus criteria. The language used (English or Indonesian) should have **ZERO** influence on the score.

    1. Source Credibility (0-10) Slightly modified to include industry-specific sources. Keywords: "Bloomberg", "Reuters", "Kontan", "Bisnis Indonesia", "Petromindo", "Dunia Tambang", "Ministry of Energy and Mineral Resources (ESDM)", "Minerba One Data", "IDX", "OJK".
    ...
    2. Clarity and Structure (0-10) No change from the original. This criterion is universal. Keywords: "clear headline", "well-structured", "organized", "informative lead".
    ...
    3. Relevance to Indonesian Coal, Metal & Mineral Sectors (0-15) Heavily modified to focus on the target industry. 
        Keywords: "coal", "nickel", "tin", "bauxite", "copper", "gold", "Adaro Energy (ADRO)", "Antam (ANTM)", "Bukit Asam (PTBA)", "Vale Indonesia (INCO)", "Merdeka Copper Gold (MDKA)", "Harum Energy (HRUM)", "Indo Tambangraya (ITMG)", "smelter", "mining concession (IUP)", "ESDM".
    ...
    4. Depth of Analysis (0-15) Modified to reflect mining industry specifics. Keywords: "production report", "exploration results", "smelter progress", "commodity price analysis", "geopolitical impact", "sector outlook", "cash cost analysis".
    ...
    5. Financial & Operational Data Inclusion (0-10) Modified to include operational metrics. Keywords: "production volume (tonnes)", "sales volume", "cash cost", "capex", "reserves", "stripping ratio", "earnings", "EBITDA", "stock price", "dividends".
    ...
    6. Balanced Reporting (0-10) No change from the original. This criterion is universal. Keywords: "balanced view", "multiple perspectives", "neutral tone", "pros and cons".
        Score 0-4: Highly biased or promotional.
        Score 4-7: Attempts balance but remains somewhat skewed.
        Score 7-10: Well-balanced, presenting multiple perspectives (e.g., company vs. environmental groups, different analyst opinions).
    7. Specific Commodity & Value Chain Focus (0-10) Heavily modified to reward specificity within the mining sector. 
        Keywords: "thermal coal", "coking coal", "nickel pig iron (NPI)", "ferronickel", "bauxite", "alumina", "copper concentrate", "tin ingot", "exploration", "downstream", "smelting", "export", "Domestic Market Obligation (DMO)".
    ...
    8. Market & Sector Impact Relevance (0-10) Modified for sector-specific impacts. Keywords: "commodity price impact", "investor sentiment", "stock price impact", "export ban", "downstream policy", "ESG concerns".
    ...
    9. Forward-Looking Statements (0-10) Modified for mining industry specifics. Keywords: "production target", "exploration plans", "future regulations", "forecast", "projections", "downstream expansion", "long-term strategy".
    ...
    for llm in LLMCOLLECTION.get_llms():
        try:
            ...
            if not response_scoring.get('news_score'):
                print('Scoring response not complete')
                continue 

            print(f'[SUCCES] Scoring for url: {article_title}')
            ...
            print(f"[Scoring] LLM failed with error: {error}")
            continue 
